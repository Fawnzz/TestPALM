...
function CreateCI() {
    ...
    //Gather controls
    var txtDescrip = document.getElementById("txtCreateDescrip");
    var txtCategory = document.getElementById("txtCreateCat");
    var txtDate = document.getElementById("txtCreateDate");
    var slcStatus = document.getElementById("slcCreateStatus");
    var slcPriority = document.getElementById("slcCreatePriority");
    var slcAudience = document.getElementById("slcCreateAudience");

    //Pass control values to variables

    var descrip = txtDescrip.value;
    var cat = txtCategory.value;
    ...
    $.ajax({
        type: "POST",
        url: "../ConfigurationItem_View.aspx/CreateCI",
        contentType: "application/json; charset=utf-8",
        data: "{description:" + JSON.stringify(descrip) + ",category:" + JSON.stringify(cat) + ",duedate:" + JSON.stringify(date) + ",status:" + JSON.stringify(status) + ",priority:" + JSON.stringify(priority) + ",audience:" + JSON.stringify(audience) + "}",
        success: function (msg) {
            alert("Configuration Item Created Successfully!!");
            location.reload();
        },
        error: function (xhr, ajaxOptions, thrownError) {
            alert(xhr.status);
...
$(".btnCreate").bind("click", Create);
$(".btnCreateCI").bind("click", CreateCI);
});